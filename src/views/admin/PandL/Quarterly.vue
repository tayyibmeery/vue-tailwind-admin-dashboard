<template>
  <PageBreadcrumb :pageTitle="'P&L - Quarterly'" />
  <div class="mb-4 flex items-center gap-3">
    <div>
      <label class="text-sm font-medium">Year:</label>
      <select v-model="year" @change="fetchData" class="rounded border p-2 dark:bg-gray-800">
        <option v-for="y in years" :key="y" :value="y">{{ y }}</option>
      </select>
    </div>
    <div>
      <label class="text-sm font-medium">Quarter:</label>
      <select v-model="quarter" @change="fetchData" class="rounded border p-2 dark:bg-gray-800">
        <option v-for="q in [1, 2, 3, 4]" :key="q" :value="q">Q{{ q }}</option>
      </select>
    </div>
  </div>
  <div v-if="loading" class="text-center py-10">Loading...</div>
  <div v-else-if="error" class="text-red-500">{{ error }}</div>
  <div v-else class="overflow-x-auto bg-white dark:bg-gray-800 rounded-xl border p-4">
    <table class="min-w-full text-sm">
      <thead>
        <tr>
          <th class="text-left">Account</th>
          <th class="text-right">Amount</th>
        </tr>
      </thead>
      <tbody>
        <tr v-for="(value, key) in data" :key="key" v-if="typeof value === 'number'">
          <td class="font-semibold">{{ key.replace(/_/g, ' ').toUpperCase() }}</td>
          <td class="text-right" :class="value < 0 ? 'text-red-500' : 'text-green-600'">{{ value.toFixed(2) }}</td>
        </tr>
      </tbody>
    </table>
  </div>
</template>

<script setup lang="ts">
import { ref, onMounted } from 'vue';
import api from '@/services/api';
import PageBreadcrumb from '@/components/common/PageBreadcrumb.vue';

const year = ref(new Date().getFullYear());
const years = Array.from({ length: 10 }, (_, i) => year.value - i);
const quarter = ref(Math.ceil(new Date().getMonth() / 3));
const data = ref<any>({});
const loading = ref(false);
const error = ref('');

async function fetchData() {
  loading.value = true;
  try {
    const res = await api.get('/admin/pandl/quarterly', { params: { year: year.value, quarter: quarter.value } });
    data.value = res.data;
  } catch (e: any) {
    error.value = e.message;
  } finally {
    loading.value = false;
  }
}

onMounted(fetchData);
</script>
